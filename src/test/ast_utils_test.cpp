...
namespace test {
    ...
    TEST(AstUtils, Grounded) {
        ...
        // X = Y
        AstLiteral* e1 = new AstConstraint("=",
    ...
    TEST(AstUtils, SimpleTypes) {
        ...
        auto getX = [](const AstClause* c) {
          return c->getHead()->getArgument(0);
        };
    ...
    TEST(AstUtils, ResolveGroundedAliases) {
        ...
        EXPECT_EQ("p(a,b) :- \n   p(x,y),\n   r = [x,y],\n   s = r,\n   s = [w,v],\n   [w,v] = [a,b].", toString(*program.getRelation("p")->getClause(0)));
    ...
    TEST(AstUtils, ResolveAliasesWithTermsInAtoms) {
        ...
        EXPECT_EQ("p(x,c) :- \n   p(x,b),\n   p(b,c),\n   c = (b+1),\n   x = (c+2).", toString(*program.getRelation("p")->getClause(0)));

        ResolveAliasesTransformer::resolveAliases(program);

        EXPECT_EQ("p(((b+1)+2),(b+1)) :- \n   p( _tmp_0,b),\n   p(b, _tmp_1),\n    _tmp_0 = ((b+1)+2),\n    _tmp_1 = (b+1).",
