...
void MainConfig::processArgs(int argc, char** argv, const std::string header, const std::string footer, const std::vector<MainOption> mainOptions) {
    ...
    // use the main options to define the global configuration
    {
        ...
        while ((c = getopt_long(argc, argv, shortNames.c_str(), longNames, nullptr)) != EOF) {
            ...
            // define the value for the option in the global configuration as its argument or an empty string if no
            // argument exists
    ...
    // obtain the name of the datalog file, and store it in the option with the empty key
    {
        ...
        // otherwise, if multiple input filenames are allowed
            ...
            // set the option in the global config for the main datalog file to all those specified by the command line arguments
            set("", filenames);
        }
    }
