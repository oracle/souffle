...
do_actual_work (std::string file_name, std::istream &instream,
      po::variables_map const &vm, bool input_is_stdin, bool warnings)
{
   ...
   try {
      ...
      bool preserve_bol_whitespace = true;
...
main (int argc, char *argv[])
{
	...
	try {
				...
				arguments[0].value[0] == "-")
		{
			...
			return do_actual_work("<stdin>", std::cin, vm, true, vm.count("warnings"));
		}
		else {
			if (arguments.size() > 1) {
			...
			return do_actual_work(file_name, instream, vm, false, vm.count("warnings"));
		}
	}
	catch (std::exception const &e) {
