...
namespace {
	...
	struct ComponentContent {
		...
		void add(std::unique_ptr<AstType>& type, ErrorReport &report) {
			...
			auto foundItem = std::find_if(types.begin(), types.end(),
					[&](const std::unique_ptr<AstType> &element) { return (element->getName() == type->getName()); });
			if(foundItem != types.end()) {
				Diagnostic err(Diagnostic::ERROR, DiagnosticMessage("Redefinition of type " + toString(type->getName()), type->getSrcLoc()),
		...
		void add(std::unique_ptr<AstRelation>& rel, ErrorReport &report) {
			// add to result content (check existence first)
			auto foundItem = std::find_if(relations.begin(), relations.end(),
	...
	ComponentContent getInstantiatedContent(const AstComponentInit &componentInit, const AstComponent *enclosingComponent, const ComponentLookup &componentLookup, std::vector<std::unique_ptr<AstClause>> &orphans, ErrorReport &report, const TypeBinding& binding, unsigned int maxDepth) {
