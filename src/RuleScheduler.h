...
namespace scheduler {
    ...
    public:
        ...
        bool isUnderscore() const { return kind == UnnamedVar; }
    ...
    public:
        ...
        Atom(int id, const std::vector<Argument>& args)
            : id(id), args(args) {}
    ...
    public:
        ...
        Problem(const std::vector<atom_type>& atoms = std::vector<atom_type>())
            : atoms(atoms) {}
        ...
        plan solve(bool debug = false) const {
            ...
            if (debug) {
                std::cout << "Results:\n" << join(cache, "\n", [](std::ostream& out, const std::pair<int,map<plan,state_type>>& cur){
                    out << cur.first << ":\n\t" << join(cur.second,"\n\t", [](std::ostream& out, const std::pair<plan,state_type>& cur){
                        out << "[" << join(cur.first, ",", [](std::ostream& out, const atom_type& atom) {
                            out << atom.getID();
                        }) << "] => " << cur.second;
                    }) << "\n";
                }) << "\n";
     ...
     */
    struct SimpleComputationalCostModel : public cost_model<SimpleComputationalCostAtom,SimpleComputationalCostState> {

        SimpleComputationalCostState applyTo(const SimpleComputationalCostState& state, const SimpleComputationalCostAtom& atom) const {
